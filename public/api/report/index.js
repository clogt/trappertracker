    ...
    try {
        ...
        if (reportData.report_type === 'dangerZone') {
            const stmt = env.DB.prepare(
                'INSERT INTO trapper_blips (latitude, longitude, report_timestamp, reported_by_user_id, description) VALUES (?, ?, ?, ?, ?) RETURNING id'
            );
            ...
            const stmt = env.DB.prepare(
                'INSERT INTO lost_pets (latitude, longitude, pet_name, species_breed, photo_url, description, owner_contact_email, time_lost, reported_by_user_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) RETURNING id'
            );
            ...
            const stmt = env.DB.prepare(
                'INSERT INTO found_pets (latitude, longitude, species_breed, photo_url, description, contact_info, time_found, reported_by_user_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?) RETURNING id'
            );
            ...
            const stmt = env.DB.prepare(
                'INSERT INTO dangerous_animals (latitude, longitude, animal_type, description, report_timestamp, reported_by_user_id) VALUES (?, ?, ?, ?, ?, ?) RETURNING id'
            );
